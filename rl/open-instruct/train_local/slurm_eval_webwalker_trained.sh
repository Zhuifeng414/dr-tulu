#!/bin/bash
#SBATCH --job-name=eval-webwalker-trained
...
#SBATCH --output=/gpfs/scrubbed/rulins/slurm_logs/dr-tulu/eval-webwalker-trained-%j.out

# WebWalker evaluation for trained checkpoint with local BM25 search
# Following pattern from test-time-training branch
...
# Configuration
# ============================================
MODEL_NAME="/gpfs/scrubbed/rulins/dr-tulu/local-bm25-8gpu/local-bm25-8gpu__1__1768371815_checkpoints/step_300"
TOKENIZER_NAME="Qwen/Qwen3-0.6B"
MODEL_TAG="trained_step300"
DATASET="webwalker"
MAX_TOOL_CALLS=20
NUM_EXAMPLES="final_run"

EVAL_OUTPUT_DIR="/gpfs/scrubbed/rulins/dr-tulu/eval_output/webwalker-${MODEL_TAG}-t${MAX_TOOL_CALLS}"
...
if [ ! -d "$MODEL_NAME" ]; then
    echo "Error: Checkpoint directory not found: $MODEL_NAME"
    exit 1
fi
...
# ============================================
# Launch MCP server with local BM25
# ============================================
echo "Starting MCP server with local BM25 on port $MCP_PORT..."
screen -dmS mcp_server bash -c "cd /gpfs/projects/kohlab/rulins/dr-tulu/rl/open-instruct && export JAVA_HOME=/gpfs/projects/kohlab/rulins/env/dev && export JVM_PATH=/gpfs/projects/kohlab/rulins/env/dev/lib/jvm/lib/server/libjvm.so && export PATH=\$JAVA_HOME/bin:\$PATH && uv run python -m dr_agent.mcp_backend.main --transport http --host 0.0.0.0 --port $MCP_PORT --local-searcher-type bm25 --index-path 'data/bm25' --dataset-name data/corpus.jsonl 2>&1 | tee /tmp/mcp_server.log"
...
uv run python /gpfs/projects/kohlab/rulins/dr-tulu/agent/workflows/auto_search_sft.py \
    ...
    --config-overrides "search_agent_model_name=$MODEL_NAME,search_agent_tokenizer_name=$TOKENIZER_NAME,search_agent_base_url=http://localhost:$MODEL_PORT/v1,mcp_port=$MCP_PORT,use_browse_agent=false,search_agent_max_tool_calls=$MAX_TOOL_CALLS,browse_tool_name=jina,search_agent_max_tokens=16000,search_agent_temperature=0.0" \
