#!/bin/bash
#SBATCH --job-name=eval-webwalker-base
...
#SBATCH --output=/gpfs/scrubbed/rulins/slurm_logs/dr-tulu/eval-webwalker-base-%j.out

# WebWalker evaluation for base model with local BM25 search
...
uv run python /gpfs/projects/kohlab/rulins/dr-tulu/agent/workflows/auto_search_sft.py \
    ...
    --config-overrides "search_agent_model_name=$MODEL_NAME,search_agent_tokenizer_name=$MODEL_NAME,search_agent_base_url=http://localhost:$MODEL_PORT/v1,mcp_port=$MCP_PORT,use_browse_agent=false,search_agent_max_tool_calls=$MAX_TOOL_CALLS,browse_tool_name=jina,search_agent_max_tokens=16000,search_agent_temperature=0.0" \
